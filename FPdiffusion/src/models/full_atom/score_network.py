...
from src.models.full_atom.diffuser.se3_diffuser import _extract_trans_rots
...
class BaseScoreNetwork(nn.Module):
    ...
    def loss_fn(
        ...
        **kwargs,
        ...
        # rotation axis loss
        rot_axis_mse = (gt_rot_axis - pred_rot_axis) ** 2 * rigids_mask[..., None]
        rot_axis_loss = torch.sum(rot_axis_mse, dim=(-2, -1)) / (rigids_mask.sum(dim=-1) + 1e-6)

        # rotation angle loss
        rot_angle_mse = (gt_rot_angle - pred_rot_angle) ** 2 * rigids_mask[..., None]
        rot_angle_loss = torch.sum(
            rot_angle_mse / gt_rot_score_norm[:, None, None] ** 2, dim=(-2, -1)
        ) / (rigids_mask.sum(dim=-1) + 1e-6)

        mean_t = t.mean(dim=1)
        rot_angle_loss *= mean_t > rot_angle_loss_t_filter
        rot_loss = rot_axis_loss + rot_angle_loss

        # translation loss
        trans_score_mse = (gt_trans_score - pred_trans_score) ** 2 * rigids_mask[..., None]
        ...
        if bb_coords_loss_weight + bb_dist_map_loss_weight > 1e-4:
            ...
            pred_atom37, _ = atom14_to_atom37(pred_atom14, aatype)
            pred_bb_coords = pred_atom37[:, :, [0, 1, 2, 4], :]  # (B, L, 4, 3)

            # backbone coordinate loss
            bb_coords_mse = (gt_bb_coords - pred_bb_coords) ** 2 * bb_coords_mask[..., None]
            bb_coords_loss = torch.sum(bb_coords_mse, dim=(-3, -2, -1)) / (
                bb_coords_mask.sum(dim=(-2, -1)) + 1e-6
            )
            bb_coords_loss *= mean_t < bb_coords_loss_t_filter
            ...
            bb_dist_map_mask = bb_coords_mask.reshape(B, 1, -1) * bb_coords_mask.reshape(B, -1, 1)
            bb_dist_map_mask *= gt_bb_dist_map < 6
            pred_bb_dist_map = torch.cdist(
                pred_bb_coords.view(B, -1, 3), pred_bb_coords.view(B, -1, 3)
            )
            bb_dist_map_mse = (gt_bb_dist_map - pred_bb_dist_map) ** 2 * bb_dist_map_mask
            bb_dist_map_loss = torch.sum(bb_dist_map_mse, dim=(-2, -1)) / (
                bb_dist_map_mask.sum(dim=(-2, -1)) + 1e-6
            )
            bb_dist_map_loss *= mean_t < bb_dist_map_loss_t_filter
        else:
            bb_coords_loss = bb_dist_map_loss = torch.zeros_like(trans_loss)

        torsion_mse = (gt_torsion_angles - pred_torsions) ** 2 * torsion_angles_mask[..., None]
        torsion_loss = torsion_mse.sum(dim=(-1, -2, -3)) / torsion_angles_mask.sum(dim=(-1, -2))
        ...
        aux_info = {
            ...
            "trans": trans_loss.mean().item(),
            "bb_coords": bb_coords_loss.mean().item(),
            "bb_dist_map": bb_dist_map_loss.mean().item(),
            "torsion": torsion_loss.mean().item(),
            "fape": fape_loss.mean().item(),
        }
    ...
    def reverse_sample(
        ...
        **kwargs,
        ...
        dt = torch.Tensor([1.0 / self.cfg.diffusion_steps] * batch_size).to(aatype.device)
        t = torch.Tensor([1.0] * batch_size).to(aatype.device)
